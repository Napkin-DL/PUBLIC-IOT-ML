...
def run(dataset_dir, small_object_area_threshold, foreground_class_of_interest):
    ...
    #    dataset directory
    coco_dir = os.path.join(dataset_dir, 'coco_dataset')
...
#     download_and_convert_visualwakewords_lib.download_coco_dataset(coco_dir)
    ...
    visualwakewords_annotations_train = os.path.join(
        dataset_dir, 'json', 'instances_visualwakewords_train2014.json')
    visualwakewords_annotations_val = os.path.join(
        dataset_dir, 'json', 'instances_visualwakewords_val2014.json')
    visualwakewords_labels_filename = os.path.join(
        dataset_dir, 'tfrecord', 'labels.txt')
    train_output_path = os.path.join(dataset_dir, 'tfrecord', 'train.record')
    val_output_path = os.path.join(dataset_dir, 'tfrecord', 'val.record')
